...
Quill SHALL exponer una abstraccion de request que permita leer de forma consistente el metodo, route params, query params, body parseado soportado y el request PSR subyacente.
...
Quill SHALL ofrecer una abstraccion de response inmutable con helpers semanticos para status, headers y formatos comunes como `json`, `plain` y `html`.
...
Quill MUST emitir la respuesta final exactamente una vez, preservando status, headers y body producidos por el pipeline.

#### Scenario: Emit final response
- **WHEN** el lifecycle del request produce una respuesta final valida
- **THEN** el emisor de respuestas transmite sus headers, status y body sin alterar el resultado observable del handler
